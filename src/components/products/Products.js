...
function Products() {
  ...
  const filteredProducts = size ? products.filter((product) => product.sizes.find((productSize) => productSize.size === parseInt(size))) : products;
        ...
        <Grid container>
          ...
          <Grid item xs={12} md={10}>
            ...
            {filteredProducts.map(product => 
              <Grid item xs={12} sm={6} md={4} key={product.id} className="product__single">
                <div>
                  ...
                  <Link className="link" to={`/products/${category}/${product.id}`}>See details</Link>
                </div>
              </Grid>
            )}
            </Grid>
